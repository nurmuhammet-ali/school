...
<div class="mt-8 bg-white rounded border-b-4 border-gray-300">
    <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
        ...
        <div class="w-4/12 px-4 py-3">Topic</div>
        <div class="w-1/12 px-4 py-3 text-center">Mark</div>
        <div class="w-2/12 px-4 py-3">Teacher</div>
        <div class="w-2/12 px-4 py-3 text-right">Action</div>
    </div>
    @foreach(App\DayLesson::where('day_id', $day->id)->get() as $lesson)
        <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
            <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $loop->iteration }}</div>
            <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $lesson->subject->name }}</div>
            ...
            <div class="w-1/12 px-4 py-3 text-sm text-gray-600 text-center tracking-tight">
                <span class="bg-gray-200 text-sm mr-1 mb-1 px-2 font-semibold border rounded-full">{{ $lesson->mark }}</span>
            </div>
            <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $lesson->teacher->user->name }}</div>
            <div class="w-2/12 flex items-center justify-end px-3">
                <a href="{{ route('classes.edit',$lesson->id) }}">
                ...
                <form action="{{ route('classes.destroy',$lesson->id) }}" method="POST" class="inline-flex ml-1">
                    @csrf
                    @method('DELETE')
                ...
                <a href="{{ route('class.assign.subject',$lesson->id) }}" class="ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm" title="assign subject">
