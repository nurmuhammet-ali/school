...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Okuw dersleri</h2>
            </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                <div class="w-5/12 px-4 py-3">Ady</div>
                <div class="w-5/12 px-4 py-3">Bellik</div>
                <div class="w-2/12 px-4 py-3 text-right"></div>
            </div>
            @foreach ($subjects as $subject)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-5/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $subject->name }}</div>
                    <div class="w-5/12 px-4 py-3 text-sm text-gray-600 tracking-tight">{{ $subject->description }}</div>
                    <div class="w-2/12 flex items-center justify-end px-3">
                        ...
                        <a href="#" data-url="{{ route('subject.destroy',$subject->id) }}" class="deletestudent ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm">
        ...
        @include('backend.modals.delete',['name' => 'Okuw dersi'])
    </div>
@endsection
